...
- `survey_agent.py`: intake agent with an `AgentTask` (`IntakeTask`) that completes once required fields are collected (Pattern 2).
...
| `survey_agent.py` | `AgentTask` (`IntakeTask`) that completes once required fields are filled | 6.2 The survey agent |
